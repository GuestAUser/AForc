...
FIELDZERO_DEPS := $(FIELDZERO_OBJECTS:.o=.d)
...
$(FIELDZERO_OBJECTS): AFORC_EXAMPLE_CPPFLAGS := $(AFORC_FIELDZERO_CPPFLAGS)
...
$(ROGUELIKE) $(FIELDZERO) $(TEST_BINARIES):
	@mkdir -p "$(@D)"
	$(CC) $(LDFLAGS) $(AFORC_LDFLAGS) $(AFORC_PROGRAM_LDFLAGS) $^ $(LDLIBS) $(AFORC_LDLIBS) -o "$@"
