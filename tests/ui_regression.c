...
static AFORC_Status
test_plot(void *context, AFORC_Point position, AFORC_Cell cell)
{
...
static bool canvas_and_widgets_are_clipped(void)
{
    ...
    if (aforc_ui_canvas_init(
            &canvas, (AFORC_Rect){1, 1, 3, 2}, test_plot, &target) !=
            AFORC_OK ||
    ...
    test_canvas_clear(&target);
    if (aforc_ui_canvas_init(
            &canvas, (AFORC_Rect){0, 0, 6, 1}, test_plot, &target) !=
            AFORC_OK ||
        aforc_ui_draw_label(&canvas,
                            (AFORC_Rect){0, 0, 6, 1},
                            label,
                            sizeof(label) - 1U,
                            AFORC_UI_ALIGN_END,
    ...
    if (aforc_ui_draw_progress(&canvas,
                               (AFORC_Rect){0, 0, 6, 1},
                               UINT64_MAX - 1U,
                               UINT64_MAX,
                               &progress) != AFORC_OK ||
...
static bool focus_and_menu_state_is_stable(void)
{
    ...
    if (aforc_ui_menu_init(&menu, items, 3U, 1U, true) != AFORC_OK ||
        menu.selected != 2U ||
        aforc_ui_menu_handle(
            &menu, items, 3U, 1U, true, AFORC_UI_EVENT_NAV_DOWN, &action) !=
            AFORC_OK ||
    ...
    if (aforc_ui_menu_handle(
            &menu, items, 3U, 1U, true, AFORC_UI_EVENT_ACTIVATE, &action) !=
            AFORC_OK ||
    ...
    if (aforc_ui_menu_handle(
            &menu, items, 3U, 1U, true, AFORC_UI_EVENT_NONE, &action) !=
            AFORC_OK ||
