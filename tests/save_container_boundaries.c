...
static bool reader_init_status(const void *container,
                               ...
                               AFORC_Status expected)
{
    ...
    const AFORC_Status status = aforc_save_reader_init(&reader,
                                                       container,
                                                       container_size,
                                                       max_payload_bytes,
                                                       minimum_schema,
...
static bool test_hostile_headers(void)
{
    ...
    mutated[0] = (uint8_t)'X';
    passed = reader_init_status(
        mutated, blob.size, sizeof(payload), 3u, 3u, AFORC_ERROR_FORMAT);

    (void)memcpy(mutated, blob.data, blob.size);
    store_u16(mutated + 4u, AFORC_SAVE_CONTAINER_VERSION + UINT16_C(1));
    refresh_header_crc(mutated);
    passed = passed && reader_init_status(mutated,
                                          blob.size,
                                          sizeof(payload),
                                          3u,
                                          3u,
    ...
    passed =
        passed &&
        reader_init_status(mutated,
                           blob.size,
                           sizeof(payload) + 1u,
                           3u,
                           3u,
                           AFORC_ERROR_FORMAT) &&
        reader_init_status(blob.data,
                           blob.size,
                           sizeof(payload) - 1u,
                           3u,
                           3u,
        ...
        reader_init_status(blob.data,
                           ...
                           4u,
                           AFORC_ERROR_UNSUPPORTED) &&
        reader_init_status(NULL, 0u, 0u, 0u, 0u, AFORC_ERROR_INVALID_ARGUMENT);
    free(mutated);
    aforc_asset_blob_release(&blob);
    return passed;
}
...
static bool test_reader_transactions(void)
{
    ...
    passed =
        aforc_save_reader_init(
            &reader, blob.data, blob.size, sizeof(truncated_string), 1u, 1u) ==
            AFORC_OK &&
...
static bool test_writer_alias_and_limits(void)
{
    ...
    {
        status =
            aforc_save_writer_write_bytes(&writer, initial, sizeof(initial));
    }
    if (status == AFORC_OK)
    {
        status = aforc_save_writer_write_string(
            &writer, (const char *)(writer.payload + 1u), 3u);
    }
    ...
    passed =
        ...
        aforc_save_writer_write_u8(&writer, UINT8_C(1)) == AFORC_ERROR_LIMIT &&
        writer.size == 4u &&
        aforc_save_writer_finish(&writer, &blob) == AFORC_OK &&
        blob.size == AFORC_SAVE_HEADER_SIZE + 4u &&
        aforc_save_writer_write_u8(&invalid, UINT8_C(1)) == AFORC_ERROR_STATE;
#if SIZE_MAX > UINT32_MAX
    passed = passed &&
             aforc_save_writer_write_string(
                 &writer, "x", (size_t)UINT32_MAX + 1u) == AFORC_ERROR_LIMIT;
#endif
    aforc_asset_blob_release(&blob);
    aforc_save_writer_release(&writer);
    return passed;
