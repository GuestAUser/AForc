...
static bool
path_equal(const AFORC_Point *left, const AFORC_Point *right, size_t length)
{
    size_t index;
...
static QueryResult legacy_query(const AFORC_TileMap *map,
                                AFORC_Point start,
                                AFORC_Point goal,
                                const AFORC_PathOptions *options)
{
    ...
    result.status = aforc_pathfind_astar(map,
                                         0U,
                                         start,
                                         goal,
                              ...
                              const AFORC_PathOptions *options)
{
    ...
    const AFORC_Status reused_status =
...
{
    AFORC_Status status = aforc_path_workspace_create(allocator, out_workspace);

    if (status == AFORC_OK)
    {
        status = aforc_path_workspace_reserve(*out_workspace, cell_capacity);
    }
    if (status != AFORC_OK)
    {
...
bool astar_test_benchmark_size(int32_t width, int32_t height, size_t iterations)
{
    ...
    const size_t cells = (size_t)(uint32_t)width * (size_t)(uint32_t)height;
    size_t length = 0U;
    size_t index;
    ...
    if (aforc_tilemap_create(
            (AFORC_Size){width, height}, 1U, 0U, &allocator, &map) !=
            AFORC_OK ||
