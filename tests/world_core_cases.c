...
static bool test_geometry(void) {
    ...
    WORLD_TEST_CHECK(aforc_world_point_equal((AFORC_Point){1, 2},
                                             (AFORC_Point){1, 2}));
    WORLD_TEST_CHECK(!aforc_world_point_equal((AFORC_Point){1, 2},
                                              (AFORC_Point){2, 1}));
    ...
    WORLD_TEST_CHECK(aforc_world_point_manhattan(
                         (AFORC_Point){INT32_MIN, INT32_MIN},
    ...
    WORLD_TEST_CHECK(aforc_world_rect_intersects(
        (AFORC_Rect){0, 0, 3, 3}, (AFORC_Rect){2, 2, 3, 3}));
    WORLD_TEST_CHECK(!aforc_world_rect_intersects(
        (AFORC_Rect){0, 0, 3, 3}, (AFORC_Rect){3, 0, 2, 2}));
    WORLD_TEST_CHECK(aforc_world_rect_translate((AFORC_Rect){1, 2, 3, 4},
                                                (AFORC_Point){-3, 5},
                                                &rect) == AFORC_OK);
    WORLD_TEST_CHECK(rect.x == -2 && rect.y == 7 && rect.width == 3 &&
                     rect.height == 4);
    WORLD_TEST_CHECK(aforc_world_rect_translate(
                         (AFORC_Rect){INT32_MAX, 0, 1, 1},
                         (AFORC_Point){1, 0}, &rect) == AFORC_ERROR_OVERFLOW);
    return true;
}
...
static bool test_tilemap_and_collision(void) {
    ...
    WORLD_TEST_CHECK(aforc_tilemap_create(
                         (AFORC_Size){INT32_MAX, INT32_MAX}, UINT32_MAX, 0U,
                         &allocator, &map) == AFORC_ERROR_OVERFLOW);
    WORLD_TEST_CHECK(map == NULL && state.attempts == attempts_before_overflow);
    WORLD_TEST_CHECK(aforc_tilemap_create((AFORC_Size){4, 3}, 2U, 7U,
    ...
    WORLD_TEST_CHECK(aforc_tilemap_fill_rect(
                         map, 0U, (AFORC_Rect){0, 0, -1, 1}, 9U) ==
                     AFORC_ERROR_INVALID_ARGUMENT);
    WORLD_TEST_CHECK(aforc_tilemap_set(map, 0U, (AFORC_Point){2, 0}, 1U) ==
                     AFORC_OK);
    WORLD_TEST_CHECK(aforc_grid_point_blocked(
                         map, 0U, (AFORC_Point){2, 0}, world_test_tile_nonzero,
                         NULL, &blocked) == AFORC_OK &&
                     blocked);
    WORLD_TEST_CHECK(aforc_grid_rect_blocked(
                         map, 0U, (AFORC_Rect){0, 0, 4, 2},
                         world_test_tile_nonzero, NULL, &blocked, &first) ==
                     AFORC_OK);
    WORLD_TEST_CHECK(blocked &&
                     aforc_world_point_equal(first, (AFORC_Point){1, 0}));
    WORLD_TEST_CHECK(aforc_grid_rect_blocked(
                         map, 0U, (AFORC_Rect){0, 0, 4, 2}, NULL, NULL,
                         &blocked, &first) == AFORC_OK &&
                     !blocked);
    WORLD_TEST_CHECK(aforc_grid_rect_blocked(
                         map, 0U, (AFORC_Rect){4, 3, 0, 0},
                         world_test_tile_nonzero, NULL, &blocked, &first) ==
                         AFORC_OK &&
                     !blocked);
...
static bool test_camera(void) {
    ...
    WORLD_TEST_CHECK(aforc_tilemap_create((AFORC_Size){10, 8}, 1U, 0U,
                                           &allocator, &map) == AFORC_OK);
    ...
    WORLD_TEST_CHECK(aforc_world_point_equal(camera.origin,
                                             (AFORC_Point){0, 5}));
    WORLD_TEST_CHECK(aforc_camera_center_on(&camera, (AFORC_Point){9, 7}, map) ==
                     AFORC_OK);
    WORLD_TEST_CHECK(aforc_world_point_equal(camera.origin,
                                             (AFORC_Point){6, 5}));
    ...
    WORLD_TEST_CHECK(view.x == 6 && view.y == 5 && view.width == 4 &&
                     view.height == 3);
    WORLD_TEST_CHECK(aforc_camera_set_viewport(&camera,
                                               (AFORC_Size){12, 10}) ==
                     AFORC_OK);
    WORLD_TEST_CHECK(aforc_camera_clamp_to_map(&camera, map) == AFORC_OK);
    WORLD_TEST_CHECK(aforc_world_point_equal(camera.origin,
