...
{
    static const AFORC_Point expected[] = {{0, 0},
                                           {1, 0},
                                           {0, 1},
                                           {1, 1},
                                           {2, 1},
    ...
    WORLD_TEST_CHECK(
        aforc_tilemap_create((AFORC_Size){5, 5}, 1U, 0U, &allocator, &map) ==
        AFORC_OK);
    WORLD_TEST_CHECK(aforc_grid_raycast(map,
                                        0U,
                                        (AFORC_Point){0, 0},
                                        (AFORC_Point){4, 4},
                                        world_test_trace_nonzero,
                                        &trace,
                                        &blocked,
                                        &hit) == AFORC_OK);
    ...
    {
        WORLD_TEST_CHECK(
            aforc_world_point_equal(trace.points[index], expected[index]));
    }
...
static bool test_fov_geometry(void)
{
    ...
    WORLD_TEST_CHECK(
        aforc_tilemap_create((AFORC_Size){7, 7}, 1U, 0U, &allocator, &map) ==
        AFORC_OK);
    (void)memset(visibility, 0xA5, sizeof(visibility));
    WORLD_TEST_CHECK(aforc_fov_compute(map,
                                       0U,
                                       (AFORC_Point){3, 3},
                                       2U,
                                       NULL,
    ...
    for (y = 0; y < 7; ++y)
    {
        for (x = 0; x < 7; ++x)
        {
            ...
            const bool expected = delta_x * delta_x + delta_y * delta_y <= 4;
...
static bool test_fov_common_case_is_allocation_free(void)
{
    ...
    WORLD_TEST_CHECK(
        aforc_tilemap_create((AFORC_Size){9, 9}, 1U, 0U, &allocator, &map) ==
        AFORC_OK);
...
{
    uint32_t value =
        x * UINT32_C(0x9e3779b1) ^ y * UINT32_C(0x85ebca6b) ^ UINT32_C(148);
...
static bool test_fov_spill_cleanup(void)
{
    ...
    WORLD_TEST_CHECK(
        aforc_tilemap_create((AFORC_Size){FOV_SPILL_EXTENT, FOV_SPILL_EXTENT},
                             1U,
                             0U,
                             &allocator,
                             &map) == AFORC_OK);
    for (y = 0; y < FOV_SPILL_EXTENT; ++y)
    {
        for (x = 0; x < FOV_SPILL_EXTENT; ++x)
        {
            ...
            {
                WORLD_TEST_CHECK(
                    aforc_tilemap_set(map, 0U, (AFORC_Point){x, y}, 1U) ==
                    AFORC_OK);
            }
        }
    }
