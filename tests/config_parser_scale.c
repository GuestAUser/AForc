...
static bool test_valid_grammar_and_order(void)
{
    ...
    const bool passed =
        status == AFORC_OK && config.count == 2u &&
...
static bool test_malformed_inputs(void)
{
    ...
    return aforc_config_scale_parse_status(missing_equals,
           ...
           aforc_config_scale_parse_status(invalid_section,
                                            ...
                                            AFORC_ERROR_FORMAT) &&
           aforc_config_scale_parse_status(invalid_cr,
                                            sizeof(invalid_cr) - 1u,
                                            AFORC_ERROR_FORMAT) &&
           aforc_config_scale_parse_status(embedded_nul,
                                            sizeof(embedded_nul),
                                            AFORC_ERROR_FORMAT);
}
...
static bool test_late_duplicate(void)
{
    ...
    passed = aforc_config_scale_parse_status(
        text,
        text_size,
        AFORC_ERROR_EXISTS);
    free(text);
...
static bool test_unique_entry_counts(void)
{
    ...
    for (index = 0u; index < sizeof(counts) / sizeof(counts[0u]); ++index) {
        ...
        passed = status == AFORC_OK && config.count == counts[index] &&
                 strcmp(config.entries[0u].section, "dense") == 0 &&
                 strcmp(config.entries[0u].key, "key0") == 0 &&
                 strcmp(config.entries[config.count - 1u].key,
                        expected_key) == 0;
        aforc_config_release(&config);
        free(text);
        if (!passed) {
