...
static uint64_t astar_oracle_cost(uint16_t mask,
                                  AFORC_Point start,
                                  AFORC_Point goal,
                                  uint32_t flags)
{
    ...
    const bool prevent = (flags & AFORC_PATH_PREVENT_CORNER_CUTTING) != 0U;
    size_t iteration;
    size_t index;
    ...
    for (iteration = 0U; iteration < ASTAR_TEST_CELLS; ++iteration)
    {
        ...
        for (direction = 0U; direction < (diagonal ? 8U : 4U); ++direction)
        {
            ...
            candidate =
                distances[best] + (offset_x != 0 && offset_y != 0 ? 14U : 10U);
    ...
    for (index = 0U; index < length; ++index)
    {
        ...
        if (index != 0U)
        {
            ...
            if (diagonal &&
                ...
                (astar_mask_blocked(
                     mask,
                     (AFORC_Point){points[index].x, points[index - 1U].y}) ||
                 astar_mask_blocked(
                     mask,
                     (AFORC_Point){points[index - 1U].x, points[index].y})))
            {
                return false;
            }
...
static bool test_exhaustive_small_maps(void)
{
    ...
    WORLD_TEST_CHECK(
        aforc_tilemap_create((AFORC_Size){3, 3}, 1U, 0U, &allocator, &map) ==
        AFORC_OK);
    WORLD_TEST_CHECK(aforc_path_workspace_create(&allocator, &workspace) ==
                     AFORC_OK);
    WORLD_TEST_CHECK(
        aforc_path_workspace_reserve(workspace, ASTAR_TEST_CELLS) == AFORC_OK);
    for (mask = 0U; mask < (UINT16_C(1) << ASTAR_TEST_CELLS); ++mask)
    {
        ...
        for (cell = 0U; cell < ASTAR_TEST_CELLS; ++cell)
        {
            ...
            {
                const AFORC_Point point = {(int32_t)(cell % ASTAR_TEST_WIDTH),
                 ...
                 ++start_index)
            {
                     ...
                     ++goal_index)
                {
                    ...
                    else
                    {
                        ...
                        WORLD_TEST_CHECK(astar_path_is_valid(mask,
                                                             &options,
                                                             points,
                                                             length,
                                                             start,
