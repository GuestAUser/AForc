...
#include <stdio.h>
#include <time.h>
bool ecs_test_create(size_t max_entities, size_t max_component_types,
                     size_t initial_component_capacity, AFORC_Ecs **out_ecs)
{
...
bool ecs_test_register_types(AFORC_Ecs *ecs, size_t type_count,
                             size_t initial_capacity, AFORC_ComponentType *types)
{
    ...
    for (index = 0U; index < type_count; ++index) {
        if (aforc_ecs_register_component(ecs, &desc, &types[index]) !=
            AFORC_OK) {
            return false;
        }
    }
...
bool ecs_test_sparse(size_t *out_sparse_bytes)
{
    ...
    for (index = 0U; passed && index < 3U; ++index) {
        ...
        passed = aforc_ecs_add(ecs, selected[index], type, &values[index],
                               NULL) == AFORC_OK;
    }
    if (!passed) {
    ...
    *out_sparse_bytes = store->sparse_capacity * sizeof(*store->sparse);
    (void)printf("sparse memory: capacity=%zu element=%zu bytes=%zu\n",
                 store->sparse_capacity, sizeof(*store->sparse),
                 *out_sparse_bytes);
    passed = sizeof(*store->sparse) == sizeof(uint32_t) &&
...
bool ecs_test_next_match(AFORC_EcsView *view, AFORC_Ecs *ecs,
                         AFORC_Entity expected, const AFORC_ComponentType *required,
                         size_t type_count)
{
...
bool ecs_test_benchmark(void)
{
    ...
    size_t entity_index;
    size_t round;
    if (!ecs_test_create(ECS_BENCHMARK_ENTITY_COUNT, ECS_TEST_TYPE_LIMIT,
                         ECS_BENCHMARK_ENTITY_COUNT, &ecs) ||
    ...
    (void)printf("benchmark memory: capacity=%zu element=%zu bytes/store=%zu\n",
                 ecs->stores[0].sparse_capacity, sizeof(*ecs->stores[0].sparse),
    ...
    for (round = 0U; round < ECS_BENCHMARK_ROUNDS; ++round) {
        ...
        void *components[ECS_TEST_TYPE_LIMIT];
        bool has_value = false;
        if (aforc_ecs_view_create(
                ecs, types, ECS_TEST_TYPE_LIMIT, &view) != AFORC_OK) {
            aforc_ecs_destroy(ecs);
            return false;
        }
        for (;;) {
            size_t type_index;
            if (aforc_ecs_view_next(
                    view, &entity, components, &has_value) != AFORC_OK) {
    ...
    (void)printf("benchmark iteration: visits=%zu elapsed_ns=%llu ns/visit=%.3f "
                 "checksum=%llu\n",
                 (size_t)ECS_BENCHMARK_ENTITY_COUNT * (size_t)ECS_BENCHMARK_ROUNDS,
                 (unsigned long long)elapsed,
                 (double)elapsed / ((double)ECS_BENCHMARK_ENTITY_COUNT *
                                    (double)ECS_BENCHMARK_ROUNDS),
