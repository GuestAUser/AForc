...
enum { RENDERER_DIFF_PARAMETER_CAPACITY = 24 };
...
{
    const AFORC_Cell cell = aforc_cell_default();
    const size_t count = (size_t)terminal->size.width * (size_t)terminal->size.height;
...
{
    static const AFORC_CellStyle styles[] = {AFORC_STYLE_NONE, AFORC_STYLE_BOLD,
        AFORC_STYLE_DIM, AFORC_STYLE_ITALIC, AFORC_STYLE_UNDERLINE,
        AFORC_STYLE_BLINK, AFORC_STYLE_NONE, AFORC_STYLE_REVERSE,
        AFORC_STYLE_HIDDEN, AFORC_STYLE_STRIKETHROUGH};
    return parameter < sizeof(styles) / sizeof(styles[0]) ? styles[parameter] : AFORC_STYLE_NONE;
}

static bool terminal_extended_color(const uint32_t *parameters, size_t parameter_count,
                                    size_t *index, AFORC_Color *output)
{
    if (*index >= parameter_count) {
        return false;
    }
    if (parameters[*index] == 5U) {
        if (*index + 1U >= parameter_count || parameters[*index + 1U] > UINT8_MAX) {
            return false;
        }
    ...
    if (parameters[*index] != 2U || *index + 3U >= parameter_count ||
        parameters[*index + 1U] > UINT8_MAX || parameters[*index + 2U] > UINT8_MAX ||
        parameters[*index + 3U] > UINT8_MAX) {
        return false;
    }
    *output = aforc_color_rgb((uint8_t)parameters[*index + 1U], (uint8_t)parameters[*index + 2U],
                              (uint8_t)parameters[*index + 3U]);
    *index += 4U;
    return true;
}

static bool terminal_sgr(TerminalModel *terminal, const uint32_t *parameters,
    ...
    for (size_t index = 0U; index < parameter_count;) {
        ...
        switch (parameter) {
            ...
            case 48U: {
                ...
                if (!terminal_extended_color(parameters, parameter_count, &index, &color)) {
                    return false;
                }
                if (parameter == 38U) {
...
static bool terminal_escape(TerminalModel *terminal, const unsigned char *bytes,
                            size_t size, size_t *consumed)
{
    ...
    while (index < size) {
        ...
        if (byte == (unsigned char)';') {
            if (!has_digits || parameter_count == RENDERER_DIFF_PARAMETER_CAPACITY) {
                return false;
            }
        ...
        *consumed = index;
        if (byte == (unsigned char)'H') {
            ...
            if (parameter_count != 2U || parameters[0] == 0U || parameters[1] == 0U ||
        ...
        return byte == (unsigned char)'m' && terminal_sgr(terminal, parameters, parameter_count);
    }
    return false;
}

static bool terminal_codepoint(const unsigned char *bytes, size_t size,
...
static bool terminal_apply(TerminalModel *terminal, const char *batch, size_t batch_size)
{
    ...
    while (offset < batch_size) {
        ...
        if (bytes[offset] == 0x1bU) {
            if (!terminal_escape(terminal, bytes + offset, batch_size - offset, &consumed)) {
                return false;
            }
        } else {
            ...
            if (!terminal_codepoint(bytes + offset, batch_size - offset, &codepoint, &consumed) ||
                terminal->row < 0 || terminal->column < 0 || terminal->row >= terminal->size.height ||
                terminal->column >= terminal->size.width) {
                return false;
            }
            cell_index = (size_t)terminal->row * (size_t)terminal->size.width + (size_t)terminal->column;
...
bool renderer_diff_apply_ansi(AFORC_Cell *cells, AFORC_Size size,
                              const char *batch, size_t batch_size)
{
