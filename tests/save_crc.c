...
static bool test_empty_vector(void)
{
    static const uint8_t expected[] = {
        0x41u, 0x32u, 0x44u, 0x53u, 0x01u, 0x00u, 0x00u,
        0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u,
        0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u,
        0x00u, 0x00u, 0x00u, 0xf0u, 0x5au, 0xaau, 0xddu,
    };
...
static bool test_known_vector(void)
{
    ...
    static const uint8_t expected[] = {
        0x41u, 0x32u, 0x44u, 0x53u, 0x01u, 0x00u, 0x00u,
        0x00u, 0x04u, 0x03u, 0x02u, 0x01u, 0x09u, 0x00u,
        0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x26u,
        0x39u, 0xf4u, 0xcbu, 0xcbu, 0x4au, 0x32u, 0x6bu,
        0x31u, 0x32u, 0x33u, 0x34u, 0x35u, 0x36u, 0x37u,
        0x38u, 0x39u,
    };
    AFORC_AssetBlob blob = {0};
...
static bool test_roundtrip(void)
{
    ...
    if (status == AFORC_OK) {
        status = aforc_save_writer_write_u64(&writer,
    ...
    if (status == AFORC_OK) {
        status = aforc_save_writer_write_string(&writer,
                                                text,
                                                sizeof(text) - 1u);
    }
    if (status == AFORC_OK) {
    ...
    if (status == AFORC_OK &&
        (value_u8 != UINT8_C(0xa5) || value_u16 != UINT16_C(0x1234) ||
         ...
         value_u64 != UINT64_C(0x0123456789abcdef) ||
         value_i32 != INT32_MIN || value_i64 != INT64_MIN ||
         view.size != sizeof(text) - 1u ||
...
static bool test_corruption_and_truncation(void)
{
    ...
    mutated[8] ^= UINT8_C(1);
    passed = aforc_save_reader_init(&reader,
                                    mutated,
                                    blob.size,
                                    blob.size,
                                    0u,
                                    UINT32_MAX) == AFORC_ERROR_CHECKSUM;
    (void)memcpy(mutated, blob.data, blob.size);
    mutated[AFORC_SAVE_HEADER_SIZE] ^= UINT8_C(1);
    passed = passed && aforc_save_reader_init(&reader,
                                              mutated,
                                              blob.size,
                                              blob.size,
                                              0u,
...
static bool test_deterministic_output(void)
{
    ...
    const bool created = make_container(11u,
                                        payload,
                                        sizeof(payload),
                                        &first) == AFORC_OK &&
                         make_container(11u,
