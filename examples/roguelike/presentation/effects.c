...
static uint64_t game_accumulate_milliseconds(double seconds, double *remainder)
{
    const double elapsed = seconds * 1000.0 + *remainder;
    const uint64_t milliseconds = (uint64_t)elapsed;

    *remainder = elapsed - (double)milliseconds;
    return milliseconds;
}
...
AFORC_Status game_emit_burst(Game *game, AFORC_Point point, bool strong)
{
    ...
    cells[1] = game_cell((uint32_t)'+',
                         ...
                         AFORC_STYLE_BOLD);
    cells[2] =
        game_cell((uint32_t)'.', aforc_color_indexed(215U), AFORC_STYLE_DIM);
    emitter.x = (AFORC_ParticleI32Range){x, x};
    emitter.y = (AFORC_ParticleI32Range){y, y};
    emitter.velocity_x = (AFORC_ParticleI32Range){-3 * AFORC_EFFECT_FIXED_ONE,
                                                  3 * AFORC_EFFECT_FIXED_ONE};
    emitter.velocity_y = (AFORC_ParticleI32Range){-2 * AFORC_EFFECT_FIXED_ONE,
                                                  2 * AFORC_EFFECT_FIXED_ONE};
    emitter.acceleration_x = (AFORC_ParticleI32Range){0, 0};
    emitter.acceleration_y =
        (AFORC_ParticleI32Range){0, AFORC_EFFECT_FIXED_ONE};
    emitter.lifetime_ms = (AFORC_ParticleU32Range){180U, 520U};
    emitter.cells = cells;
    emitter.cell_count = sizeof(cells) / sizeof(cells[0]);
    status = aforc_particle_pool_emit(
        &game->particle_pool, &emitter, burst_count, &spawned);
                                     ...
                                     AFORC_Error *error)
{
    ...
    const uint32_t milliseconds = (uint32_t)game_accumulate_milliseconds(
        seconds, &game->particle_millisecond_remainder);
    AFORC_Status status;
    ...
    if (status != AFORC_OK)
    {
        return game_error(error, status, "effects", "particle update failed");
    }
    return AFORC_OK;
}
...
{
    Game *game = scene->user_data;
    const uint64_t milliseconds = game_accumulate_milliseconds(
        seconds, &game->tween_millisecond_remainder);
    AFORC_Status status;
    ...
    if (status != AFORC_OK)
    {
        return game_error(error, status, "effects", "exit tween update failed");
    }
    return AFORC_OK;
}
