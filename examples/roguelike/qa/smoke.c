    ...
    for (int32_t y = 0; y < size.height; ++y)
    {
        for (int32_t x = 0; x + (int32_t)length <= size.width; ++x)
        {
            ...
            while (index < length)
            {
                ...
                if (aforc_renderer_get(game->renderer,
                                          ...
                                          AFORC_Error *error)
{
    ...
    while (status == AFORC_OK)
    {
        ...
        if (isolate)
        {
            position->point = (AFORC_Point){
                game->rules.map_width - 2 - (int32_t)(relocated % 16U),
                game->rules.map_height - 2 - (int32_t)(relocated / 16U)};
            actor->hostile = false;
            ++relocated;
        }
    }
    aforc_ecs_view_destroy(view);
    if (status == AFORC_OK && (out_enemies->sentinel_actor == NULL ||
                               out_enemies->goblin_actor == NULL))
    {
                                                   ...
                                                   AFORC_Error *error)
{
    ...
    AFORC_Status status = game_smoke_archetypes(game, false, &enemies, error);
                                                     ...
                                                     AFORC_Error *error)
{
    ...
    AFORC_Status status = aforc_renderer_resize(
        game->renderer, (AFORC_Size){GAME_MIN_COLUMNS - 1, GAME_MIN_ROWS});
                                                        ...
                                                        AFORC_Error *error)
{
    ...
    {
        status =
            aforc_input_feed(game->input, burst, sizeof(burst) - 1U, 1001U);
    }
    if (status == AFORC_OK)
    {
    ...
    if (status == AFORC_OK && (game->turn != turn + 1U || remaining == 0U))
    {
        status = game_error(error,
                            AFORC_ERROR_STATE,
                            "smoke",
...
static AFORC_Status game_smoke_enemy_perception(Game *game, AFORC_Error *error)
{
    GameSmokeArchetypes enemies;
    GamePosition *player_position = NULL;
    GameActor *player_actor = NULL;
    ...
    {
        status =
            aforc_tilemap_set(game->map, 0U, (AFORC_Point){x, 2}, TILE_FLOOR);
        if (status == AFORC_OK)
        {
            status = aforc_tilemap_set(
                game->map, 0U, (AFORC_Point){x, 4}, TILE_FLOOR);
        }
    }
    for (int32_t y = 2; status == AFORC_OK && y <= 4; ++y)
    {
        status =
            aforc_tilemap_set(game->map, 0U, (AFORC_Point){12, y}, TILE_FLOOR);
    }
    if (status == AFORC_OK)
    {
    ...
    if (status == AFORC_OK)
    {
        ...
        status =
            aforc_tilemap_set(game->map, 0U, (AFORC_Point){6, 2}, TILE_WALL);
    }
    if (status == AFORC_OK)
    {
        status = game_enemy_turns(game);
...
static AFORC_Status game_smoke_turn_feedback(Game *game, AFORC_Error *error)
{
    GameSmokeArchetypes enemies;
    GamePosition *player_position = NULL;
    GameActor *player_actor = NULL;
    ...
    {
        status =
            aforc_tilemap_set(game->map, 0U, (AFORC_Point){10, 10}, TILE_FLOOR);
    }
    if (status == AFORC_OK)
    {
        status =
            aforc_tilemap_set(game->map, 0U, (AFORC_Point){11, 10}, TILE_FLOOR);
    }
    if (status == AFORC_OK)
    {
        ...
        enemies.sentinel_actor->health = enemies.sentinel_actor->maximum_health;
        enemies.goblin_actor->hostile = false;
        status = game_move_player(game, (AFORC_Point){1, 0});
    }
    if (status == AFORC_OK && (strstr(game->message, "You strike") == NULL ||
...
static AFORC_Status
game_smoke_adaptive_render(Game *game, AFORC_Engine *engine, AFORC_Error *error)
{
    const AFORC_Size saved_size = aforc_renderer_size(game->renderer);
    const AFORC_Size test_size = {game->rules.map_width + 8,
                                  game->rules.map_height + GAME_HUD_ROWS + 4};
    GamePosition *position = NULL;
    GameActor *actor = NULL;
    AFORC_Point saved_position;
    AFORC_Cell player_cell;
    AFORC_Status status =
    ...
    status = aforc_renderer_resize(game->renderer, test_size);
    if (status == AFORC_OK)
    {
    ...
    if (status == AFORC_OK)
    {
        status = aforc_renderer_get(
            game->renderer,
            (AFORC_Point){saved_position.x + 4, saved_position.y + 2},
            &player_cell);
    }
    if (status == AFORC_OK &&
        (player_cell.codepoint != (uint32_t)'@' || !game->visibility_valid ||
         !aforc_world_point_equal(game->visibility_origin, saved_position)))
    {
        status =
            game_error(error,
                       AFORC_ERROR_STATE,
                       "smoke",
                       "adaptive world render was not centered or visible");
    }
    ...
    if (status == AFORC_OK && (!game->visibility_valid ||
                               !aforc_world_point_equal(game->visibility_origin,
                                                        game->exit_position)))
    {
        status = game_error(error,
                            AFORC_ERROR_STATE,
                            "smoke",
                            "visibility cache did not follow the player");
    }
                                                ...
                                                AFORC_Error *error)
{
    ...
    status = game_scene_render(&game->scene, engine, 0.0, error);
    if (status == AFORC_OK && !game_smoke_renderer_contains(game, "HP "))
    {
        status = game_error(error,
                            AFORC_ERROR_STATE,
                            "smoke",
                                                         ...
                                                         AFORC_Error *error)
{
    ...
    {
        status =
            aforc_particle_pool_spawn(&game->particle_pool, &particle, NULL);
    }
    if (status == AFORC_OK)
    {
...
static AFORC_Status game_smoke_score_saturates(Game *game, AFORC_Error *error)
{
    GamePosition *position = NULL;
    GameActor *actor = NULL;
    AFORC_Point saved_position;
...
game_smoke_checks(Game *game, AFORC_Engine *engine, AFORC_Error *error)
{
    ...
    status = game_actor_components(game, game->player, &position, &actor);
    if (status == AFORC_OK)
    {
        ...
        path_length <= game->cell_count)
    {
        ...
        status = aforc_alloc_array(
            &game->allocator, path_capacity, sizeof(*path), (void **)&path);
    }
    if (status == AFORC_OK)
    {
        status = aforc_pathfind_astar(game->map,
    ...
    if (status == AFORC_OK &&
        (game->seed != saved_seed || game->floor != saved_floor ||
         ...
         game->particle_pool.active_count != 0U ||
    ...
    if (status == AFORC_OK)
    {
        status = game_smoke_burst_preserves_particles(
            game, position->point, false, error);
    }
    if (status == AFORC_OK)
    {
        status = game_smoke_burst_preserves_particles(
            game, position->point, true, error);
    }
    if (status == AFORC_OK)
    {
        status = game_emit_burst(game, position->point, true);
    ...
    if (status == AFORC_OK)
    {
        status = aforc_input_feed(
            game->input, wait_input, sizeof(wait_input) - 1U, 1U);
    }
    if (status == AFORC_OK)
    {
    ...
    if (status == AFORC_OK)
    {
        ...
        status =
            aforc_renderer_get(game->renderer,
