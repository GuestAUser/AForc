...
static AFORC_Status game_smoke_gather_enemies(
    Game *game,
...
static bool game_smoke_snapshots_equal(
    const GameSmokeActorSnapshot *left,
...
static AFORC_Status game_smoke_prepare_exact_source(Game *source) {
    ...
    AFORC_Status status = game_generate_floor(source,
                                              2U,
                                              source->rules.player_health);
...
static AFORC_Status game_smoke_exact_save_round_trip(Game *game,
    ...
    if (status == AFORC_OK) {
        status = game_smoke_gather_enemies(&source,
                                           source_enemies,
                                           &source_enemy_count);
    }
    if (status == AFORC_OK) {
        status = game_smoke_gather_enemies(&loaded,
                                           loaded_enemies,
                                           &loaded_enemy_count);
    }
    if (status == AFORC_OK &&
        (source.seed != loaded.seed || source.floor != loaded.floor ||
         ...
         !game_smoke_snapshots_equal(source_enemies,
                                     loaded_enemies,
                                     source_enemy_count))) {
    ...
    if (status == AFORC_OK) {
        ...
        if (status == AFORC_OK) {
            const AFORC_Status decode_status = game_decode_save(
                &loaded,
                hostile_blob.data,
                hostile_blob.size);
            ...
            if (decode_status != AFORC_ERROR_FORMAT ||
                ...
                loaded.seed != previous_seed || loaded.floor != previous_floor ||
                loaded.score != previous_score || loaded.turn != previous_turn ||
...
static AFORC_Status game_smoke_legacy_save(Game *game, AFORC_Error *error) {
    ...
    const int32_t expected_health = game->rules.player_health < health
                                        ? game->rules.player_health
                                        : health;
    ...
    if (status == AFORC_OK &&
         (loaded.seed != seed || loaded.floor != floor ||
