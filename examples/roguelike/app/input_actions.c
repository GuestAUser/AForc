                                                   ...
                                                   AFORC_Status status)
{
    ...
    game_set_message(
        game, "%s failed: %s.", operation, aforc_status_string(status));
    return AFORC_OK;
}
    ...
    *out_consumed = true;
    ...
    if (codepoint == (uint32_t)'?')
    {
        if (!event->data.key.repeat)
        {
            ...
            game_set_message(
                game, game->help_visible ? "Help opened." : "Help closed.");
        }
        return AFORC_OK;
    }
    ...
    if (game->run_state != GAME_PLAYING)
    {
        ...
        game_set_message(game,
                         "Press L to load, R for a new run, or Q to quit.");
        return AFORC_OK;
    }
                              ...
                              AFORC_Error *error)
{
    ...
    *consumed = false;
    ...
    {
        return game_error(
            error, status, "roguelike", "could not process input event");
    }
    return AFORC_OK;
}
