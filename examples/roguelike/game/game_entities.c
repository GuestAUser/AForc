                               ...
                               AFORC_Entity *out_entity)
{
    ...
    if (status == AFORC_OK)
    {
        status = aforc_ecs_add(
            game->ecs, entity, game->position_type, &component_position, NULL);
    }
    if (status == AFORC_OK)
    {
        status =
            aforc_ecs_add(game->ecs, entity, game->actor_type, &actor, NULL);
    }
    if (status != AFORC_OK)
    {
                                   ...
                                   GameActor **out_actor)
{
    ...
    status = aforc_ecs_get(game->ecs, entity, game->position_type, &position);
    if (status == AFORC_OK)
    {
