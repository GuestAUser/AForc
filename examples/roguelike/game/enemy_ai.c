...
AFORC_Status game_enemy_turns(Game *game)
{
     ...
     */
    status = game_gather_enemies(game, enemies, GAME_MAX_ENEMIES, &enemy_count);
    for (size_t index = 0U; status == AFORC_OK && index < enemy_count; ++index)
    {
        ...
        status = game_actor_components(
            game, enemies[index], &enemy_position, &enemy_actor);
        if (status == AFORC_OK)
        {
            status = game_actor_components(
        ...
        else if (distance <= game_enemy_perception(enemy_actor))
        {
            ...
            {
                status = aforc_pathfind_astar_workspace(game->path_workspace,
                                                        game->map,
                                                        0U,
                                                        enemy_position->point,
                                                        player_position->point,
            ...
            if (status == AFORC_OK && path_length > 1U)
            {
                ...
                status = game_entity_at(game, path[1], &occupant, &occupied);
