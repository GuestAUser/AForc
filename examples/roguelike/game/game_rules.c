...
static const char game_configuration[] = "[game]\n"
                                         "map_width=72\n"
                                         "map_height=36\n"
                                         "room_count=15\n"
                                         "enemy_count=12\n"
...
static AFORC_Status game_rule(const AFORC_Config *config,
                              ...
                              uint32_t *out_value)
{
    return game_parse_u32(
        aforc_config_get(config, "game", key), minimum, maximum, out_value);
}
...
AFORC_Status game_load_rules(GameRules *rules)
{
    ...
    status = aforc_config_parse(
        game_configuration, sizeof(game_configuration) - 1U, &limits, &config);
    if (status == AFORC_OK)
    {
    ...
    {
        status = game_rule(
            &config, "room_count", 4U, GAME_MAX_ROOMS, &rules->room_count);
    }
    if (status == AFORC_OK)
    {
    ...
    if (status == AFORC_OK)
    {
        status = game_rule(&config, "player_health", 1U, 1000U, &player_health);
    }
    if (status == AFORC_OK)
    {
        status = game_rule(&config, "player_attack", 1U, 100U, &player_attack);
    }
    if (status == AFORC_OK)
    {
        status = game_rule(&config, "enemy_health", 1U, 100U, &enemy_health);
    }
    if (status == AFORC_OK)
    {
        status = game_rule(&config, "enemy_attack", 1U, 100U, &enemy_attack);
    }
    if (status == AFORC_OK)
    {
        status =
            game_rule(&config, "final_floor", 1U, 20U, &rules->final_floor);
    }
