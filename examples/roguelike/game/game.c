...
void game_dispose(Game *game)
{
    ...
    aforc_ecs_view_destroy(game->render_view);
                             ...
                             uint64_t seed)
{
    ...
    status =
        aforc_path_workspace_create(&game->allocator, &game->path_workspace);
    if (status == AFORC_OK)
    {
    ...
    if (status == AFORC_OK)
    {
        status = aforc_ecs_register_component(
            game->ecs, &position_desc, &game->position_type);
    }
    if (status == AFORC_OK)
    {
        status = aforc_ecs_register_component(
            game->ecs, &actor_desc, &game->actor_type);
    }
    if (status == AFORC_OK)
    {
        ...
        status = aforc_ecs_view_create(game->ecs,
                                       types,
    ...
    if (status == AFORC_OK)
    {
        status = aforc_tween_init(
            &game->exit_tween, 0.0, 1.0, 900U, AFORC_EASING_QUADRATIC_IN_OUT);
    }
    if (status == AFORC_OK)
    {
