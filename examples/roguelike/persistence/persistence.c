...
static AFORC_Status game_save_write_actor(AFORC_SaveWriter *writer,
                                          const GameSaveActorSnapshot *snapshot)
{
    AFORC_Status status =
        aforc_save_writer_write_i32(writer, snapshot->point.x);
    ...
    {
        status =
            aforc_save_writer_write_i32(writer, snapshot->actor.maximum_health);
    }
    if (status == AFORC_OK)
    {
    ...
    if (status == AFORC_OK)
    {
        status = aforc_save_writer_write_u32(
            writer, (uint32_t)snapshot->actor.color.mode);
    }
    if (status == AFORC_OK)
    {
        status = aforc_save_writer_write_u8(writer, snapshot->actor.color.red);
    }
    if (status == AFORC_OK)
    {
        status =
            aforc_save_writer_write_u8(writer, snapshot->actor.color.green);
    }
    if (status == AFORC_OK)
    {
        status = aforc_save_writer_write_u8(writer, snapshot->actor.color.blue);
    }
    if (status == AFORC_OK)
    {
...
static AFORC_Status game_save_read_actor(AFORC_SaveReader *reader,
                                         GameSaveActorSnapshot *snapshot)
{
    uint32_t color_mode = 0U;
    uint8_t hostile = 0U;
    AFORC_Status status =
        aforc_save_reader_read_i32(reader, &snapshot->point.x);
    ...
    {
        status =
            aforc_save_reader_read_i32(reader, &snapshot->actor.maximum_health);
    }
    if (status == AFORC_OK)
    {
    ...
    {
        status =
            aforc_save_reader_read_u8(reader, &snapshot->actor.color.green);
    }
    if (status == AFORC_OK)
    {
...
static AFORC_Status game_save_capture_enemies(const Game *game,
                                              ...
                                              size_t *out_count)
{
    ...
    if (status == AFORC_OK && aforc_ecs_entity_count(game->ecs) != count + 1U)
    {
...
AFORC_Status game_encode_save(const Game *game, AFORC_AssetBlob *out_blob)
{
    ...
    AFORC_Status status = aforc_ecs_get_const(
        game->ecs, game->player, game->position_type, &position_component);

    if (status == AFORC_OK)
    {
        status = aforc_ecs_get_const(
            game->ecs, game->player, game->actor_type, &actor_component);
    }
    if (status == AFORC_OK)
    {
        status =
            aforc_tilemap_get_dimensions(game->map, &dimensions, &layer_count);
    }
    if (status == AFORC_OK &&
        (dimensions.width <= 0 || dimensions.height <= 0 || layer_count != 1U ||
    ...
    if (status == AFORC_OK)
    {
        status = aforc_save_writer_init(
            &writer, GAME_SAVE_SCHEMA, GAME_SAVE_MAX_BYTES);
    }
    if (status == AFORC_OK)
    {
    ...
    {
        status =
            aforc_save_writer_write_u32(&writer, (uint32_t)dimensions.width);
    }
    if (status == AFORC_OK)
    {
        status =
            aforc_save_writer_write_u32(&writer, (uint32_t)dimensions.height);
    }
    if (status == AFORC_OK)
    {
        status =
            aforc_save_writer_write_u32(&writer, (uint32_t)game->cell_count);
    }
    if (status == AFORC_OK)
    {
        status = aforc_save_writer_write_bytes(
            &writer, game->explored, game->cell_count);
    }
    if (status == AFORC_OK)
    {
...
static AFORC_Status
game_save_read_exact(Game *game, AFORC_SaveReader *reader, GameSaveState *state)
{
    ...
    if (status == AFORC_OK)
    {
        status = aforc_save_reader_read_bytes(
            reader, state->explored, state->explored_count);
    }
    if (status == AFORC_OK)
    {
    ...
    for (size_t index = 0U; status == AFORC_OK && index < state->enemy_count;
         ++index)
    {
...
static AFORC_Status game_save_validate_tiles(const Game *game,
                                             const GameSaveState *state)
{
    AFORC_Tile tile = TILE_WALL;
    AFORC_Status status =
        aforc_tilemap_get(game->map, 0U, state->player.point, &tile);
    ...
    for (size_t index = 0U; status == AFORC_OK && index < state->enemy_count;
         ++index)
    {
        status = aforc_tilemap_get(
            game->map, 0U, state->enemies[index].point, &tile);
        if (status == AFORC_OK && tile == TILE_WALL)
        {
            return AFORC_ERROR_FORMAT;
        }
...
static AFORC_Status game_replace_exact(Game *game, const GameSaveState *state)
{
    Game replacement = {0};
    const char *save_path = game->save_path;
    AFORC_Status status = game_initialize(
        &replacement, game->renderer, game->input, game->terminal, state->seed);
    ...
    status = game_generate_floor(
        &replacement, state->floor, state->player.actor.health);
    if (status == AFORC_OK)
    {
    ...
    (void)memcpy(replacement.explored, state->explored, state->explored_count);
...
AFORC_Status game_load(Game *game)
{
    ...
    status = aforc_asset_load_binary(
        ".", game->save_path, &policy, GAME_SAVE_MAX_FILE_BYTES, &blob);
    if (status == AFORC_OK)
    {
