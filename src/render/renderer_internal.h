...
static inline bool aforc_renderer_cell_is_valid(AFORC_Cell cell)
{
    ...
    const bool printable =
        cell.codepoint >= UINT32_C(0x20) &&
        !(cell.codepoint >= UINT32_C(0x7f) && cell.codepoint < UINT32_C(0xa0));
...
static inline void
aforc_renderer_fill_cells(AFORC_Cell *cells, size_t count, AFORC_Cell cell)
{
    size_t index;
...
aforc_renderer_ansi_literal(AFORC_Renderer *renderer, const char *literal);
AFORC_INTERNAL AFORC_Status aforc_renderer_ansi_cursor(AFORC_Renderer *renderer,
                                                       uint32_t row,
                                                       uint32_t column);
AFORC_INTERNAL AFORC_Status aforc_renderer_ansi_style(AFORC_Renderer *renderer,
                                                      AFORC_Cell cell);
AFORC_INTERNAL AFORC_Status
aforc_renderer_ansi_codepoint(AFORC_Renderer *renderer, uint32_t codepoint);
/* Caller guarantees that every cell contains a printable ASCII codepoint. */
AFORC_INTERNAL AFORC_Status aforc_renderer_ansi_ascii_cells(
    AFORC_Renderer *renderer, const AFORC_Cell *cells, size_t count);
