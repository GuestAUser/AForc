...
static AFORC_Status ansi_reserve(AFORC_Renderer *renderer, size_t additional)
{
    ...
    capacity =
        renderer->batch_capacity == 0u ? 4096u : renderer->batch_capacity;
...
static AFORC_Status
ansi_append(AFORC_Renderer *renderer, const void *data, size_t size)
{
...
AFORC_Status aforc_renderer_ansi_ascii_cells(AFORC_Renderer *renderer,
                                             const AFORC_Cell *cells,
                                             size_t count)
{
    const AFORC_Status status = ansi_reserve(renderer, count);
    ...
    {
        renderer->batch[renderer->batch_size + index] =
            (char)cells[index].codepoint;
    }
    renderer->batch_size += count;
    return AFORC_OK;
}
...
static AFORC_Status
ansi_color(AFORC_Renderer *renderer, AFORC_Color color, bool foreground)
{
    AFORC_Status status;
