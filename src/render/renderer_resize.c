...
AFORC_Status aforc_renderer_resize(AFORC_Renderer *renderer, AFORC_Size size)
{
    ...
    status = aforc_alloc_array(
        &renderer->allocator, count, sizeof(*front), (void **)&front);
    if (status == AFORC_OK)
    {
        status = aforc_alloc_array(
            &renderer->allocator, count, sizeof(*back), (void **)&back);
    }
    if (status != AFORC_OK)
    {
    ...
    copy_width =
        renderer->size.width < size.width ? renderer->size.width : size.width;
    copy_height = renderer->size.height < size.height ? renderer->size.height
                                                      : size.height;
    for (row = 0; row < copy_height; ++row)
    {
        const size_t old_offset = (size_t)row * (size_t)renderer->size.width;
