...
static size_t first_enabled(const AFORC_UIMenuItem *items, size_t item_count)
{
...
static size_t last_enabled(const AFORC_UIMenuItem *items, size_t item_count)
{
...
static size_t
enabled_from(const AFORC_UIMenuItem *items, size_t item_count, size_t start)
{
    size_t index = start;
                                  ...
                                  AFORC_UIAction *out_action)
{
    ...
    if (state->selected != AFORC_UI_NO_INDEX && !items[state->selected].enabled)
    {
    ...
    else if (event == AFORC_UI_EVENT_NAV_DOWN)
    {
        target =
            state->selected == AFORC_UI_NO_INDEX
                ...
                : next_enabled(
                      items, item_count, state->selected, state->wrap, true);
    ...
    else if (event == AFORC_UI_EVENT_NAV_UP)
    {
        target =
            state->selected == AFORC_UI_NO_INDEX
                ...
                : next_enabled(
                      items, item_count, state->selected, state->wrap, false);
        if (target == AFORC_UI_NO_INDEX)
        {
            target = state->selected;
        }
