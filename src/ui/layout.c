...
AFORC_Status aforc_ui_layout_split(AFORC_Rect bounds,
    ...
    limit = (uint64_t)(axis == AFORC_UI_LAYOUT_ROW ? bounds.width
                                                 : bounds.height);
    ...
    offset = index64 * base +
             (index64 < remainder ? index64 : remainder) +
             index64 * (uint32_t)gap;
    extent = base + (index64 < remainder ? 1U : 0U);
    position = (axis == AFORC_UI_LAYOUT_ROW ? bounds.x : bounds.y) +
               (int64_t)offset;
    if (position > INT32_MAX) {
        return AFORC_ERROR_OVERFLOW;
    }
...
AFORC_Status aforc_ui_layout_anchor(AFORC_Rect bounds,
    ...
    horizontal_offset = column == 0U
                            ? 0
                            : column == 1U
                                  ? (bounds.width - size.width) / 2
                                  : bounds.width - size.width;
    vertical_offset = row == 0U
                          ? 0
                          : row == 1U
