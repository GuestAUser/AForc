...
static void aforc_terminal_restore_runtime(AFORC_Terminal *terminal)
{
    ...
    if (terminal->termios_changed) {
        (void)tcsetattr(terminal->config.input_fd,
                        TCSAFLUSH,
                        &terminal->original_termios);
        terminal->termios_changed = false;
    }
    if (terminal->flags_changed) {
        (void)fcntl(terminal->config.input_fd,
                    F_SETFL,
                    terminal->original_input_flags);
                             ...
                             const AFORC_TerminalConfig *config)
{
    ...
    *out_terminal = NULL;
    ...
    if (tcgetattr(terminal->config.input_fd,
...
{
    ssize_t count;
    const size_t request = capacity > (size_t)SSIZE_MAX
                               ? (size_t)SSIZE_MAX
                               : capacity;
...
AFORC_Status aforc_terminal_write(AFORC_Terminal *terminal,
                              const void *data,
                              size_t size)
{
