...
static inline size_t aforc_particle_free_word_count(
...
static inline bool aforc_particle_slot_is_free(
    const AFORC_ParticlePool *pool,
    size_t index) {
...
static inline size_t aforc_particle_find_free_from(
    const AFORC_ParticlePool *pool,
    size_t start) {
    ...
    for (;;) {
        if (word != 0U) {
            const size_t index =
                word_index * AFORC_PARTICLE_INDEX_BITS +
...
static inline bool aforc_particle_pool_size_output_aliases_state(
    ...
    first_word_address =
        (uintptr_t)(const void *)&pool->particles[0].free_bits;
    last_word_address =
    ...
    return output_address >= first_word_address &&
           ...
           (output_address - first_word_address) % sizeof(AFORC_Particle) ==
               0U;
}
...
static inline AFORC_Status aforc_particle_pool_activate_free(
