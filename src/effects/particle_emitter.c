...
static AFORC_Status emitter_validate(const AFORC_ParticleEmitter *emitter)
{
    if (emitter == NULL || !i32_range_valid(emitter->x) ||
        !i32_range_valid(emitter->y) || !i32_range_valid(emitter->velocity_x) ||
...
static size_t random_index(AFORC_ParticlePool *pool, size_t count)
{
...
#if SIZE_MAX > UINT32_MAX
    {
        const uint64_t value =
            ((uint64_t)random_next(pool) << 32U) | random_next(pool);
                                      ...
                                      size_t *out_spawned_count)
{
    ...
    if (pool == NULL || out_spawned_count == NULL ||
        aforc_particle_pool_size_output_aliases_state(pool, out_spawned_count))
    {
        return AFORC_ERROR_INVALID_ARGUMENT;
    }
    *out_spawned_count = 0U;
    ...
    for (size_t count = 0U; count < emit_count; ++count)
    {
        ...
        description.cell =
            emitter->cells[random_index(pool, emitter->cell_count)];
        status = aforc_particle_pool_activate_free(
            pool, &description, &particle_index);
        if (status != AFORC_OK)
        {
