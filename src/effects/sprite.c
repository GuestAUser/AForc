                               ...
                               size_t stride)
{
    ...
    if (size.height > 1 && stride > SIZE_MAX / ((size_t)size.height - 1U))
    {
                               ...
                               void *context)
{
    ...
    if (options->transform.scale_x == 0U || options->transform.scale_y == 0U ||
    ...
    unrotated_width =
        (int64_t)sprite->size.width * (int64_t)options->transform.scale_x;
    unrotated_height =
        (int64_t)sprite->size.height * (int64_t)options->transform.scale_y;
    ...
    {
        const int64_t clip_right =
            (int64_t)options->clip.x + options->clip.width;
        const int64_t clip_bottom =
            (int64_t)options->clip.y + options->clip.height;
    ...
    for (int64_t y = top; y < bottom; ++y)
    {
        for (int64_t x = left; x < right; ++x)
        {
            ...
            source_x =
                (uint32_t)(unrotated_x / (int64_t)options->transform.scale_x);
            source_y =
                (uint32_t)(unrotated_y / (int64_t)options->transform.scale_y);
