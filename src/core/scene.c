...
static AFORC_Status scene_error(AFORC_Error *error, AFORC_Status status,
                              const char *message) {
    aforc_error_set(error, status, "scene", "%s", message);
    return status;
}

AFORC_Status aforc_scene_stack_init(AFORC_SceneStack *stack, size_t capacity,
                                AFORC_Allocator allocator, AFORC_Error *error) {
    AFORC_Status status;
    if (stack == NULL || capacity == 0U || allocator.allocate == NULL ||
        allocator.reallocate == NULL || allocator.deallocate == NULL) {
        return scene_error(error, AFORC_ERROR_INVALID_ARGUMENT,
                           "invalid scene stack configuration");
    }
    memset(stack, 0, sizeof(*stack));
    stack->allocator = allocator;
    status = aforc_alloc_array(&allocator, capacity, sizeof(*stack->items),
                             (void **)&stack->items);
    if (status != AFORC_OK) {
...
static AFORC_Status enter_scene(AFORC_Scene *previous, AFORC_Scene *next,
                              AFORC_Engine *engine, AFORC_Error *error) {
...
AFORC_Status aforc_scene_stack_push(AFORC_SceneStack *stack, AFORC_Engine *engine,
                                AFORC_Scene *scene, AFORC_Error *error) {
    AFORC_Scene *previous;
    AFORC_Status status;
    if (!stack_can_mutate(stack) || scene == NULL || scene->vtable == NULL) {
        return scene_error(error, AFORC_ERROR_STATE,
...
AFORC_Status aforc_scene_stack_pop(AFORC_SceneStack *stack, AFORC_Engine *engine,
                               AFORC_Scene **out_scene, AFORC_Error *error) {
    AFORC_Scene *removed;
    ...
    if (stack->count == 0U) {
        return scene_error(error, AFORC_ERROR_NOT_FOUND, "scene stack is empty");
    }
...
AFORC_Status aforc_scene_stack_replace(AFORC_SceneStack *stack, AFORC_Engine *engine,
                                    AFORC_Scene *scene, AFORC_Error *error) {
    AFORC_Scene *previous;
    AFORC_Status status;
    if (!stack_can_mutate(stack) || scene == NULL || scene->vtable == NULL) {
        return scene_error(error, AFORC_ERROR_STATE,
                           "scene replacement has invalid state");
    }
    if (stack->count == 0U) {
...
AFORC_Scene *aforc_scene_stack_top(const AFORC_SceneStack *stack) {
    return stack == NULL || stack->count == 0U ? NULL
...
static AFORC_Status dispatch_frames(AFORC_SceneStack *stack, AFORC_Engine *engine,
                                  double value, DispatchKind kind,
                                  AFORC_Error *error) {
    size_t first;
    AFORC_Status status = AFORC_OK;
    if (stack == NULL || stack->items == NULL || stack->dispatching) {
        return scene_error(error, AFORC_ERROR_STATE, "invalid scene dispatch state");
    }
    if (stack->count == 0U) {
        return AFORC_OK;
    }
    ...
    while (first > 0U) {
        ...
        const uint32_t flag = kind == DISPATCH_RENDER ? AFORC_SCENE_RENDER_BELOW
...
AFORC_Status aforc_scene_stack_update(AFORC_SceneStack *stack, AFORC_Engine *engine,
                                  double seconds, AFORC_Error *error) {
    return dispatch_frames(stack, engine, seconds, DISPATCH_UPDATE, error);
}

AFORC_Status aforc_scene_stack_render(AFORC_SceneStack *stack, AFORC_Engine *engine,
                                  double interpolation, AFORC_Error *error) {
    return dispatch_frames(stack, engine, interpolation, DISPATCH_RENDER, error);
}

AFORC_Status aforc_scene_stack_event(AFORC_SceneStack *stack, AFORC_Engine *engine,
    ...
    if (stack == NULL || stack->items == NULL || stack->dispatching ||
        event == NULL) {
        return scene_error(error, AFORC_ERROR_STATE, "invalid event dispatch state");
    }
    stack->dispatching = true;
    for (size_t index = stack->count; index > 0U; --index) {
        AFORC_Scene *scene = stack->items[index - 1U];
        if (scene->vtable->event != NULL) {
            status = scene->vtable->event(scene, engine, event, &handled, error);
