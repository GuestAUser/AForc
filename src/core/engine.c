...
static void default_log(void *context,
                                 ...
                                 AFORC_Error *error)
{
    ...
    *out_engine = NULL;
    ...
    status = aforc_alloc_array(
        &config->allocator, 1U, sizeof(*engine), (void **)&engine);
    if (status != AFORC_OK)
    {
        return aforc_engine_set_error(
            error, status, "could not allocate engine");
    }
    ...
    status = aforc_scene_stack_init(
        &engine->scenes, config->scene_capacity, config->allocator, error);
    if (status != AFORC_OK)
    {
        aforc_free(&config->allocator, engine);
        return status;
    ...
    if (status != AFORC_OK)
    {
        ...
        aforc_free(&config->allocator, engine);
        return aforc_engine_set_error(
            error, status, "could not allocate scene commands");
    }
...
static AFORC_Status
invoke_hook(AFORC_EngineHookFn hook, AFORC_Engine *engine, AFORC_Error *error)
{
...
static AFORC_Status
engine_frame_impl(AFORC_Engine *engine, uint64_t now_ns, AFORC_Error *error)
{
    ...
    else
    {
        ...
        engine->last_time_ns = now_ns;
        aforc_log(&engine->config.logger,
                  AFORC_LOG_WARNING,
                  "engine",
    ...
    {
        status = aforc_scene_stack_fixed_update(&engine->scenes,
                                                engine,
    ...
    status = aforc_scene_stack_update(
        &engine->scenes, engine, (double)elapsed_ns / 1.0e9, error);
    if (status != AFORC_OK)
    {
        return status;
    }
    ...
    engine->interpolation =
        (double)engine->accumulator_ns / (double)engine->fixed_step_ns;
    status = aforc_scene_stack_render(
        &engine->scenes, engine, engine->interpolation, error);
    if (status != AFORC_OK)
    {
        return status;
    }
...
AFORC_Status
aforc_engine_frame(AFORC_Engine *engine, uint64_t now_ns, AFORC_Error *error)
{
    AFORC_Status status;
    ...
    {
        return aforc_engine_set_error(
            error, AFORC_ERROR_STATE, "engine frame is already owned");
    }
...
AFORC_Status aforc_engine_run(AFORC_Engine *engine, AFORC_Error *error)
{
    ...
    while (!engine->quit_requested)
    {
        ...
        const uint64_t frame_start =
            engine->config.hooks.now(engine->config.hooks.context);
            ...
            engine->config.hooks.sleep != NULL)
        {
            const uint64_t budget =
                UINT64_C(1000000000) / engine->config.target_frames_per_second;
            const uint64_t end =
                engine->config.hooks.now(engine->config.hooks.context);
...
AFORC_Status aforc_engine_dispatch_event(AFORC_Engine *engine,
                                         const void *event,
                                         bool *consumed,
                                         AFORC_Error *error)
{
    ...
    return aforc_scene_stack_event(
        &engine->scenes, engine, event, consumed, error);
}
