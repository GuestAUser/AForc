...
static bool path_u64_multiply(uint64_t left, uint64_t right,
...
static AFORC_Point path_point_from_index(const AFORC_TileMap *map,
                                         size_t index) {
...
AFORC_Status aforc_path_workspace_create(
    ...
    *out_workspace = NULL;
    status = aforc_alloc_array(allocator, 1U, sizeof(*workspace),
                               (void **)&workspace);
    if (status != AFORC_OK) {
        return status;
    }
...
AFORC_Status aforc_path_workspace_reserve(AFORC_PathWorkspace *workspace,
    ...
    status = aforc_alloc_array(&workspace->allocator, cell_capacity,
...
static bool path_node_less(const PathHeap *heap, size_t left_index,
                           size_t right_index) {
...
static void path_heap_sift_up(PathHeap *heap, size_t position) {
    while (position > 0U) {
        ...
        if (!path_node_less(heap, heap->indices[position],
                            heap->indices[parent])) {
            break;
        }
...
static AFORC_Status path_heuristic(const AFORC_TileMap *map,
                                   size_t from_index, size_t goal_index,
                                   bool diagonal, uint64_t *out_heuristic) {
...
static AFORC_Status path_write(const AFORC_TileMap *map,
                               const PathNode *nodes, size_t start_index,
                               size_t goal_index, AFORC_Point *out_points,
                               size_t point_capacity, size_t *out_length) {
...
static AFORC_Status path_request_prepare(const AFORC_TileMap *map,
                                         uint32_t layer, AFORC_Point start,
                                         AFORC_Point goal,
    ...
    if (aforc_world_tile_matches_internal(map, layer, start, is_blocked,
                                          context)) {
...
static AFORC_Status path_search(AFORC_PathWorkspace *workspace,
                                const AFORC_TileMap *map, uint32_t layer,
    ...
    status = path_heuristic(map, start_index, goal_index, request->diagonal,
    ...
    while (heap.count != 0U) {
        ...
        if (node_index == goal_index) {
            return path_write(map, workspace->nodes, start_index, goal_index,
                              out_points, point_capacity, out_length);
        }
        ...
        for (direction_index = 0U; direction_index < direction_count;
            ...
            if (aforc_world_tile_matches_internal(map, layer, neighbor,
                                                  is_blocked, context)) {
                continue;
            }
            step_cost = offset_x != 0 && offset_y != 0
                            ? PATH_DIAGONAL_COST
                            : PATH_CARDINAL_COST;
            if (!path_u64_add(workspace->nodes[node_index].cost, step_cost,
                              &candidate_cost)) {
                return AFORC_ERROR_OVERFLOW;
            }
...
AFORC_Status aforc_pathfind_astar_workspace(
    AFORC_PathWorkspace *workspace, const AFORC_TileMap *map, uint32_t layer,
    AFORC_Point start, AFORC_Point goal, AFORC_TileTestFn is_blocked,
    void *context, const AFORC_PathOptions *options,
    const AFORC_Point **out_points, size_t *out_length) {
    PathRequest request;
    AFORC_Status status;
    ...
    *out_points = NULL;
    *out_length = 0U;
    status = path_request_prepare(map, layer, start, goal, is_blocked, context,
                                  options, &request);
    if (status != AFORC_OK) {
        return status;
    }
    ...
    status = path_search(workspace, map, layer, start, goal, is_blocked,
                         context, &request, workspace->path_points,
                         workspace->capacity, out_length);
    if (status == AFORC_OK) {
        *out_points = workspace->path_points;
    }
    return status;
...
AFORC_Status aforc_pathfind_astar(const AFORC_TileMap *map, uint32_t layer,
                                  AFORC_Point start, AFORC_Point goal,
                                  AFORC_TileTestFn is_blocked, void *context,
                                  const AFORC_PathOptions *options,
                                  AFORC_Point *out_points,
    ...
    if (out_length == NULL ||
        (out_points == NULL && point_capacity != 0U)) {
        return AFORC_ERROR_INVALID_ARGUMENT;
    }
    *out_length = 0U;
    status = path_request_prepare(map, layer, start, goal, is_blocked, context,
    ...
    status = aforc_alloc_array(&workspace.allocator, workspace.capacity,
    ...
    (void)memset(workspace.nodes, 0,
                 workspace.capacity * sizeof(*workspace.nodes));
    status = path_search(&workspace, map, layer, start, goal, is_blocked,
                         context, &request, out_points, point_capacity,
                         out_length);
    aforc_free(&workspace.allocator, workspace.heap_indices);
    aforc_free(&workspace.allocator, workspace.nodes);
    return status;
