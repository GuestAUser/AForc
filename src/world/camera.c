...
static int32_t world_camera_clamp_axis(int64_t value,
                                       int32_t map_extent,
                                       int32_t viewport_extent)
{
    const int32_t maximum =
        map_extent > viewport_extent ? map_extent - viewport_extent : 0;
    if (value < 0)
    {
        return 0;
    }
...
AFORC_Status aforc_camera_set_viewport(AFORC_Camera *camera,
                                       AFORC_Size viewport)
{
    if (camera == NULL || !camera_viewport_is_valid(viewport))
    {
...
AFORC_Status aforc_camera_center_on(AFORC_Camera *camera,
                                    AFORC_Point target,
                                    const AFORC_TileMap *map)
{
    int64_t candidate_x;
                                          ...
                                          AFORC_Point *out_screen)
{
    ...
    return aforc_world_point_from_i64_internal(
        ...
        (int64_t)world.y - camera->origin.y,
        out_screen);
}
                                          ...
                                          AFORC_Point *out_world)
{
    ...
    return aforc_world_point_from_i64_internal(
        ...
        (int64_t)screen.y + camera->origin.y,
        out_world);
}
