...
AFORC_Status aforc_world_point_from_i64_internal(int64_t x, int64_t y,
...
size_t aforc_world_tile_index_internal(const AFORC_TileMap *map, uint32_t layer,
                                     AFORC_Point point) {
...
AFORC_Status aforc_world_validate_layer_point_internal(const AFORC_TileMap *map,
    ...
    const AFORC_Status status =
        aforc_world_validate_layer_internal(map, layer);
    if (status != AFORC_OK) {
        return status;
    }
...
bool aforc_world_tile_matches_internal(const AFORC_TileMap *map, uint32_t layer,
                                     AFORC_Point point,
