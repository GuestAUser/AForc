...
AFORC_Status aforc_world_point_add(AFORC_Point left, AFORC_Point right,
                               AFORC_Point *out_sum) {
    return aforc_world_point_from_i64_internal(
        (int64_t)left.x + (int64_t)right.x,
        (int64_t)left.y + (int64_t)right.y, out_sum);
}
...
AFORC_Status aforc_world_rect_translate(AFORC_Rect rect, AFORC_Point delta,
                                    AFORC_Rect *out_rect) {
