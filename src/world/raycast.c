...
AFORC_Status aforc_grid_raycast(const AFORC_TileMap *map,
                                uint32_t layer,
                                AFORC_Point start,
                                AFORC_Point end,
                                ...
                                AFORC_Point *out_hit)
{
    ...
    *out_blocked = false;
    ...
    y = start.y;
    delta_x =
        end.x >= start.x ? (int64_t)end.x - start.x : (int64_t)start.x - end.x;
    delta_y =
        end.y >= start.y ? (int64_t)end.y - start.y : (int64_t)start.y - end.y;
    ...
    for (;;)
    {
        ...
        if (((steps_x * 2) + 1) * delta_y == ((steps_y * 2) + 1) * delta_x)
        {
        ...
        else if (((steps_x * 2) + 1) * delta_y < ((steps_y * 2) + 1) * delta_x)
        {
            x += step_x;
            ++steps_x;
        }
