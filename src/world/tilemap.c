...
AFORC_Status aforc_tilemap_create(AFORC_Size size, uint32_t layer_count,
    ...
    *out_map = NULL;
...
#endif
    if (!aforc_size_multiply((size_t)size.width, (size_t)size.height,
                           &cell_count) ||
    ...
    status = aforc_alloc_array(allocator, tile_count, sizeof(*map->tiles),
                             (void **)&map->tiles);
    if (status != AFORC_OK) {
...
AFORC_Status aforc_tilemap_get(const AFORC_TileMap *map, uint32_t layer,
                           AFORC_Point point, AFORC_Tile *out_tile) {
    AFORC_Status status;
    ...
    *out_tile =
        map->tiles[aforc_world_tile_index_internal(map, layer, point)];
    return AFORC_OK;
}

AFORC_Status aforc_tilemap_set(AFORC_TileMap *map, uint32_t layer,
                           AFORC_Point point, AFORC_Tile tile) {
...
AFORC_Status aforc_tilemap_fill_layer(AFORC_TileMap *map, uint32_t layer,
                                  AFORC_Tile tile) {
...
AFORC_Status aforc_tilemap_fill_rect(AFORC_TileMap *map, uint32_t layer,
                                 AFORC_Rect rect, AFORC_Tile tile) {
    AFORC_Status status =
