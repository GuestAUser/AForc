...
AFORC_Status aforc_input_create(
...
)
{
    ...
    *out_input = NULL;
    ...
    input->key_release_timeout_ms =
        effective_config.key_release_timeout_ms;
...
)
{
    ...
    input->queue_overflowed = false;
    effective_timeout = aforc_input_internal_effective_timeout(
        input,
        timeout_ms,
        timestamp_ms
    );
    status = aforc_terminal_poll(
        terminal,
        effective_timeout,
        &readable,
        &resized
    ...
    if (readable) {
        for (;;) {
            ...
            status = aforc_input_internal_feed(
                input,
                buffer,
                count,
                timestamp_ms
...
AFORC_Status aforc_input_flush(
    AFORC_Input *input,
    uint64_t timestamp_ms
)
{
...
bool aforc_input_mouse_pressed(
    const AFORC_Input *input,
    AFORC_MouseButton button
)
{
...
bool aforc_input_mouse_released(
    const AFORC_Input *input,
    AFORC_MouseButton button
)
{
