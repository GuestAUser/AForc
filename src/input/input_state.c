...
static uint64_t aforc_input_internal_add_timeout(uint64_t now_ms,
                                                 uint32_t timeout_ms)
{
...
static int aforc_input_internal_timeout_until(uint64_t deadline,
                                              uint64_t now_ms)
{
...
void aforc_input_internal_emit_text(AFORC_Input *input,
                                    uint32_t codepoint,
                                    uint64_t timestamp_ms)
{
    AFORC_InputEvent event =
        aforc_input_internal_event(AFORC_INPUT_EVENT_TEXT, timestamp_ms);

    event.data.text.codepoint = codepoint;
    (void)aforc_input_internal_queue_event(input, &event);
}

void aforc_input_internal_emit_key_down(AFORC_Input *input,
                                        AFORC_Key key,
                                        uint32_t codepoint,
                                        ...
                                        uint64_t timestamp_ms)
{
    ...
    if (aforc_input_internal_key_valid(key))
    {
        ...
        AFORC_InputEvent event = aforc_input_internal_event(
            AFORC_INPUT_EVENT_KEY_DOWN, timestamp_ms);
        ...
        state->expires_at_ms =
            explicit_release ? 0U
                             : aforc_input_internal_add_timeout(
                                   timestamp_ms, input->key_release_timeout_ms);
    ...
    if (emit_text && codepoint != 0u &&
        (modifiers & (AFORC_MOD_ALT | AFORC_MOD_CTRL | AFORC_MOD_SUPER)) == 0u)
    {
        aforc_input_internal_emit_text(input, codepoint, timestamp_ms);
    }
}

void aforc_input_internal_emit_key_up(AFORC_Input *input,
                                      AFORC_Key key,
                                      uint32_t codepoint,
                                      AFORC_Modifiers modifiers,
                                      uint64_t timestamp_ms)
...
void aforc_input_internal_release_expired(AFORC_Input *input,
                                          uint64_t timestamp_ms)
{
    size_t index = 1u;
    ...
    for (index = 1u; index < AFORC_KEY_COUNT; ++index)
    {
            ...
            input->keys[index].expires_at_ms <= timestamp_ms)
        {
            aforc_input_internal_emit_key_up(
                input, (AFORC_Key)index, 0u, AFORC_MOD_NONE, timestamp_ms);
        }
    }
}

void aforc_input_internal_emit_mouse_button(AFORC_Input *input,
                                            ...
                                            uint64_t timestamp_ms)
{
    ...
    if (down)
    {
        ...
        state->held = true;
        event = aforc_input_internal_event(AFORC_INPUT_EVENT_MOUSE_DOWN,
                                           timestamp_ms);
    }
    else
    {
        state->held = false;
        state->released = true;
        event = aforc_input_internal_event(AFORC_INPUT_EVENT_MOUSE_UP,
                                           timestamp_ms);
    }
...
void aforc_input_internal_emit_mouse_move(AFORC_Input *input,
                                          AFORC_MouseButton button,
                                          AFORC_Modifiers modifiers,
                                          uint64_t timestamp_ms)
{
    AFORC_InputEvent event =
        aforc_input_internal_event(AFORC_INPUT_EVENT_MOUSE_MOVE, timestamp_ms);
...
void aforc_input_internal_release_mouse_buttons(AFORC_Input *input,
                                                AFORC_Modifiers modifiers,
                                                uint64_t timestamp_ms)
{
    size_t index = 1u;
    ...
    for (index = 1u; index < AFORC_MOUSE_BUTTON_COUNT; ++index)
    {
        ...
        {
            aforc_input_internal_emit_mouse_button(input,
...
void aforc_input_internal_release_all(AFORC_Input *input, uint64_t timestamp_ms)
{
    size_t index = 1u;
    ...
    aforc_input_internal_release_mouse_buttons(
        input, AFORC_MOD_NONE, timestamp_ms);
}

int aforc_input_internal_effective_timeout(const AFORC_Input *input,
                                           int timeout_ms,
                                           uint64_t now_ms)
{
    ...
    if (input->escape_pending)
    {
        const uint64_t deadline = aforc_input_internal_add_timeout(
            input->escape_started_ms, input->escape_timeout_ms);
        const int remaining =
            aforc_input_internal_timeout_until(deadline, now_ms);
    ...
    for (index = 1u; index < AFORC_KEY_COUNT; ++index)
    {
        if (input->keys[index].held && !input->keys[index].explicit_release)
        {
            const int remaining = aforc_input_internal_timeout_until(
                input->keys[index].expires_at_ms, now_ms);
