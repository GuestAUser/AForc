...
static bool aforc_string_matches_range(const char *string,
...
static size_t aforc_config_pair_hash(AFORC_ConfigRange section,
                                     AFORC_ConfigRange key)
{
    ...
    return section_hash ^ (key_hash + (size_t)UINT64_C(0x9e3779b97f4a7c15) +
...
bool aforc_config_index_contains(const AFORC_ConfigIndex *index,
...
void aforc_config_index_insert(AFORC_ConfigIndex *index,
                               const AFORC_ConfigEntry *entries,
                               size_t entry_index)
{
...
AFORC_Status aforc_config_index_reserve(AFORC_ConfigIndex *index,
                                        const AFORC_ConfigEntry *entries,
                                        size_t count)
{
    size_t required;
    ...
    if (!aforc_size_multiply(
            next_capacity, sizeof(*next_slots), &allocation_size))
    {
        return AFORC_ERROR_LIMIT;
    }
