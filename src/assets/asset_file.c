...
static AFORC_Status
aforc_stream_write_all(FILE *stream, const void *data, size_t size)
{
...
static AFORC_Status aforc_stream_load(FILE *stream,
                                      size_t max_bytes,
                                      bool text_mode,
                                      uint8_t **output_data,
                                      size_t *output_size)
{
    ...
    *output_size = 0u;
        ...
        if (size == capacity)
        {
            ...
            if (!aforc_size_add(capacity, 1u, &required) ||
                !aforc_assets_growth_capacity(
                    capacity, required, max_bytes, 4096u, &next_capacity))
            {
                status = AFORC_ERROR_LIMIT;
                break;
            }
    ...
    /* Empty binary blobs deliberately carry no allocation; empty text still
     * does. */
    ...
    if (status == AFORC_OK && text_mode)
    {
        ...
        if (status == AFORC_OK && size > 0u && memchr(data, '\0', size) != NULL)
        {
...
static AFORC_Status aforc_asset_load_file(const char *root,
                                          ...
                                          size_t *output_size)
{
    ...
    *output_size = 0u;
    ...
    status = aforc_stream_load(
        stream, max_bytes, text_mode, output_data, output_size);
...
AFORC_Status aforc_asset_load_binary(const char *root,
                                     const char *relative_path,
                                     const AFORC_AssetPathPolicy *policy,
                                     size_t max_bytes,
                                     AFORC_AssetBlob *output)
{
    ...
    output->data = NULL;
    output->size = 0u;
    return aforc_asset_load_file(root,
                                 relative_path,
                                 policy,
                                 max_bytes,
                                 false,
...
AFORC_Status aforc_asset_load_text(const char *root,
                                   const char *relative_path,
                                   const AFORC_AssetPathPolicy *policy,
                                   size_t max_bytes,
                                   AFORC_AssetText *output)
{
    ...
    status = aforc_asset_load_file(
        root, relative_path, policy, max_bytes, true, &data, &output->size);
    if (status == AFORC_OK)
    {
...
AFORC_Status aforc_asset_store_binary(const char *root,
                                      const char *relative_path,
                                      const AFORC_AssetPathPolicy *policy,
                                      const void *data,
                                      size_t size,
                                      size_t max_bytes)
{
    ...
    {
        return size > max_bytes ? AFORC_ERROR_LIMIT
                                ...
                                size_t max_bytes)
{
    ...
    if (!aforc_size_add(
            directory_prefix_size, sizeof(temporary_name), &temporary_size))
    {
        free(path);
        return AFORC_ERROR_LIMIT;
    }
