...
AFORC_API AFORC_Status aforc_asset_path_validate(
    const char *relative_path, const AFORC_AssetPathPolicy *policy);
AFORC_API AFORC_Status
aforc_asset_path_join(const char *root,
                      const char *relative_path,
                      const AFORC_AssetPathPolicy *policy,
                      char *destination,
                      size_t destination_capacity);
AFORC_API AFORC_Status
aforc_asset_load_binary(const char *root,
                        const char *relative_path,
                        const AFORC_AssetPathPolicy *policy,
                        size_t max_bytes,
                        AFORC_AssetBlob *output);
AFORC_API AFORC_Status
aforc_asset_load_text(const char *root,
                      const char *relative_path,
                      const AFORC_AssetPathPolicy *policy,
                      size_t max_bytes,
...
AFORC_API AFORC_Status
aforc_asset_store_binary(const char *root,
                         const char *relative_path,
                         const AFORC_AssetPathPolicy *policy,
                         const void *data,
                         size_t size,
 ...
 */
AFORC_API AFORC_Status
aforc_asset_store_binary_atomic(const char *root,
                                const char *relative_path,
                                const AFORC_AssetPathPolicy *policy,
                                const void *data,
                                size_t size,
...
AFORC_API AFORC_Status aforc_rng_seed(AFORC_Rng *rng,
                                      uint64_t seed,
                                      uint64_t stream);
AFORC_API AFORC_Status aforc_rng_next_u32(AFORC_Rng *rng, uint32_t *output);
AFORC_API AFORC_Status aforc_rng_bounded_u32(AFORC_Rng *rng,
...
AFORC_API AFORC_Status aforc_config_parse(const char *input,
                                          size_t input_size,
                                          const AFORC_ConfigLimits *limits,
                                          AFORC_Config *output);
AFORC_API const char *aforc_config_get(const AFORC_Config *config,
                                       const char *section,
                                       const char *key);
 ...
 */
AFORC_API AFORC_Status aforc_save_writer_init(AFORC_SaveWriter *writer,
                                              uint32_t schema_version,
                                              size_t max_payload_bytes);
AFORC_API void aforc_save_writer_release(AFORC_SaveWriter *writer);
AFORC_API AFORC_Status aforc_save_writer_write_u8(AFORC_SaveWriter *writer,
                                                  uint8_t value);
AFORC_API AFORC_Status aforc_save_writer_write_u16(AFORC_SaveWriter *writer,
                                                   uint16_t value);
AFORC_API AFORC_Status aforc_save_writer_write_u32(AFORC_SaveWriter *writer,
                                                   uint32_t value);
AFORC_API AFORC_Status aforc_save_writer_write_u64(AFORC_SaveWriter *writer,
                                                   uint64_t value);
AFORC_API AFORC_Status aforc_save_writer_write_i32(AFORC_SaveWriter *writer,
                                                   int32_t value);
AFORC_API AFORC_Status aforc_save_writer_write_i64(AFORC_SaveWriter *writer,
                                                   int64_t value);
AFORC_API AFORC_Status aforc_save_writer_write_bytes(AFORC_SaveWriter *writer,
                                                     const void *data,
                                                     size_t size);
AFORC_API AFORC_Status aforc_save_writer_write_string(AFORC_SaveWriter *writer,
                                                      const char *text,
                                                      size_t size);
AFORC_API AFORC_Status aforc_save_writer_finish(const AFORC_SaveWriter *writer,
                                                AFORC_AssetBlob *output);

AFORC_API AFORC_Status aforc_save_reader_init(AFORC_SaveReader *reader,
...
AFORC_API AFORC_Status aforc_save_reader_read_u8(AFORC_SaveReader *reader,
                                                 uint8_t *output);
AFORC_API AFORC_Status aforc_save_reader_read_u16(AFORC_SaveReader *reader,
                                                  uint16_t *output);
AFORC_API AFORC_Status aforc_save_reader_read_u32(AFORC_SaveReader *reader,
                                                  uint32_t *output);
AFORC_API AFORC_Status aforc_save_reader_read_u64(AFORC_SaveReader *reader,
                                                  uint64_t *output);
AFORC_API AFORC_Status aforc_save_reader_read_i32(AFORC_SaveReader *reader,
                                                  int32_t *output);
AFORC_API AFORC_Status aforc_save_reader_read_i64(AFORC_SaveReader *reader,
                                                  int64_t *output);
AFORC_API AFORC_Status aforc_save_reader_read_bytes(AFORC_SaveReader *reader,
                                                    void *destination,
                                                    size_t size);
AFORC_API AFORC_Status aforc_save_reader_read_string(AFORC_SaveReader *reader,
