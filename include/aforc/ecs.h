...
#define AFORC_ENTITY_INVALID \
    AFORC_Entity{AFORC_ENTITY_INVALID_INDEX, UINT32_C(0)}
 ...
 * entity remain queryable for the duration of the callback. The callback may
 * inspect the ECS, but must not mutate or destroy it; mutating calls return an
 * invalid-state status while cleanup is active.
 */
 ...
 * descriptor with zero alignment uses _Alignof(max_align_t), and zero initial
 * capacity uses the ECS-wide component reservation. The allocator and its
 * context must remain valid until the ECS and all derived views are destroyed.
 */
...
AFORC_API bool aforc_ecs_entity_alive(const AFORC_Ecs *ecs, AFORC_Entity entity);
...
AFORC_API AFORC_Status aforc_ecs_reserve_entities(AFORC_Ecs *ecs, size_t capacity);
 ...
 */
AFORC_API AFORC_Status aforc_ecs_register_component(
    AFORC_Ecs *ecs,
...
/*
 * initial_value supplies exactly the registered component size. NULL requests
 * zero-initialization. out_component is optional. Returned pointers are
 * ECS-owned and may be invalidated by any structural ECS mutation; never free
 * or retain them across such a mutation.
 */
 ...
 * Structural mutation invalidates a view, after which next returns an
 * invalid-state status. out_components, when non-NULL, must provide one slot
 * per required type in the same order supplied at creation.
 */
AFORC_API AFORC_Status aforc_ecs_view_create(
    AFORC_Ecs *ecs,
